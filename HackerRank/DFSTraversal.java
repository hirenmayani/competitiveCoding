...
public class DFSTraversal {
	...
	private static void dfs_rec(ArrayList<ArrayList<Integer>> adjLists, boolean[] nodeExpanded, int v) {
		nodeExpanded[v] = true;
		System.out.print(v + " ");
		for (int w : adjLists.get(v)) {
			if (!nodeExpanded[w]) {
				dfs_rec(adjLists, nodeExpanded, w);
			}
		}
	}
	...
	public static void dfs(ArrayList<ArrayList<Integer>> adjLists, int s) {
		int n = adjLists.size();
		boolean[] nodeExpanded = new boolean[n];
		dfs_rec(adjLists, nodeExpanded, s);
	}
